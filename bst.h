...
uint Node<T>::depth() const {
	...
	if (this == NULL){
		...
		int left_depth = le->depth();
		int right_depth = ri->depth();
...
// =================================================================
std::string printCurrentLevel(Node<int> *root, int level){
	string val;
	if(root == NULL)
		return "";
	if (level == 1){
		val = std::to_string(root->value);
		return val + " ";
	}
	else if(level > 1){
